...
import { MessageCircle, X, Send, Minimize2, Bot, ChevronRight } from 'lucide-react'
...
const KB = {
  ...
  faqs: [
    { q: /batal|cancel|refund|kembal/i, a: 'Pembatalan dapat dilakukan maksimal 7 hari sebelum keberangkatan. Refund 100% untuk pembatalan ≥14 hari, 50% untuk 7–13 hari. Hubungi kami di +62 811-2345-678.' },
    { q: /bayar|payment|cicil|dp|transfer/i, a: 'Kami menerima pembayaran via Transfer Bank (BCA, Mandiri, BRI), Virtual Account, dan cicilan. DP minimal 30% dari total harga untuk konfirmasi booking.' },
    { q: /asuransi|insurance|kecelakaan/i, a: 'Semua paket wisata sudah termasuk asuransi perjalanan dari Tokio Marine. Anda bisa menambah coverage asuransi premium dengan biaya tambahan.' },
    { q: /guide|pemandu|tour leader/i, a: 'Setiap paket sudah termasuk pemandu wisata profesional berpengalaman minimal 5 tahun, fasih berbahasa Indonesia dan Inggris.' },
    { q: /visa|passport|paspor/i, a: 'Untuk paket domestik tidak diperlukan paspor. Untuk paket internasional, Anda perlu menyiapkan paspor berlaku minimal 6 bulan. Kami dapat membantu proses visa dengan biaya tambahan.' },
    { q: /hotel|penginapan|akomodasi/i, a: 'Paket standar sudah termasuk hotel bintang 3. Upgrade ke bintang 4-5 tersedia dengan biaya tambahan. Semua hotel telah kami seleksi untuk kenyamanan dan keamanan Anda.' },
    { q: /makanan|makan|meal|food/i, a: 'Paket kami sudah termasuk makan 3x sehari (sarapan, makan siang, makan malam) dengan pilihan menu lokal yang autentik.' },
    { q: /berapa orang|minimum|minimal|grup|group/i, a: 'Minimal 2 orang untuk paket private tour. Paket group sharing mulai dari 1 orang (bergabung dengan peserta lain). Untuk grup korporat/sekolah (>20 orang) tersedia paket khusus.' },
  ],
}
...
async function generateResponse(input, tours, context) {
  ...
  // ── GREETING ──
  if (/^(halo|hi|hai|hello|selamat|good|pagi|siang|sore|malam|hey|assalam|hei)\b/i.test(text)) {
    ...
    return {
      ...
      quickReplies: ['Lihat paket wisata', 'Cara pemesanan', 'Daftar mitra B2B', 'Info perusahaan'],
      context: 'greeting',
    }
  }
  ...
  // ── PAKET WISATA (all) ──
  if (/paket|wisata|tour|destinasi|semua|list|daftar paket/i.test(text) && !/cara|bagaimana|how|proses/i.test(text)) {
    if (tours.length === 0) {
      return { text: 'Maaf, saat ini belum ada paket wisata tersedia. Silakan hubungi kami langsung di +62 811-2345-678.', quickReplies: ['Hubungi kami'], context: null }
    }
    const list = tours.slice(0, 5).map((t, i) =>
  ...
  // ── DESTINASI SPESIFIK ──
  const destMatch = tours.find(t =>
    text.includes(t.destination.toLowerCase()) || text.includes(t.title.toLowerCase())
  )
  if (destMatch) {
    return {
  ...
  if (/harga|biaya|cost|price|murah|mahal|berapa|tarif/i.test(text)) {
    if (tours.length > 0) {
      ...
      return {
        ...
        quickReplies: ['Lihat semua paket', 'Cara pemesanan', 'Metode pembayaran'],
        context: 'price',
      }
    }
  }
  ...
  // ── CARA BOOKING / PESAN ──
  if (/pesan|booking|reservasi|cara|bagaimana|how|proses pemesanan/i.test(text) && !/b2b|mitra|agen/i.test(text)) {
    return {
  ...
  // ── B2B / TRAVEL AGENT ──
  if (/b2b|mitra|travel agent|agen|partner|kemitraan|daftar mitra|bisnis|korporat/i.test(text)) {
    return {
  ...
  // ── KONTAK ──
  if (/kontak|hubungi|telepon|phone|whatsapp|wa|email|alamat|kantor|lokasi/i.test(text)) {
    return {
  ...
  // ── TENTANG PERUSAHAAN ──
  if (/tentang|about|siapa|company|perusahaan|profil|amoures|sejarah|berdiri/i.test(text)) {
    return {
  ...
  if (/bantuan|help|bisa apa|apa saja|menu|pilihan/i.test(text)) {
    return {
      ...
      quickReplies: ['Lihat paket wisata', 'Cara pemesanan', 'Daftar mitra B2B', 'Hubungi kami'],
      context: 'help',
    }
  }
  ...
  return {
    ...
    quickReplies: ['Lihat paket wisata', 'Cara pemesanan', 'Hubungi kami', 'Bantuan'],
    context: null,
  }
}
...
function Message({ msg }) {
  ...
  return (
    <div className={`flex items-end gap-2 ${isBot ? 'justify-start' : 'justify-end'} mb-3`}>
      {isBot && (
      ...
      <div className={`max-w-[82%] ${isBot ? 'order-2' : 'order-1'}`}>
        ...
        <p className={`text-[10px] text-gray-400 mt-0.5 ${isBot ? 'text-left pl-1' : 'text-right pr-1'}`}>
          {msg.time}
        </p>
      </div>
    </div>
...
export default function ChatBot() {
  ...
  useEffect(() => {
    supabase
      .from('tour_packages')
      .select('id,title,destination,description,price,duration_days,quota,departure_date,is_active')
      .eq('is_active', true)
  ...
  return (
    <>
      ...
      {open && (
        <div
          className={`fixed bottom-24 right-4 sm:right-6 z-[9999] w-[calc(100vw-2rem)] sm:w-96 transition-all duration-300 ${
            minimized ? 'opacity-0 pointer-events-none translate-y-4' : 'opacity-100 translate-y-0'
          }`}
        >
          <div className="bg-white rounded-3xl shadow-2xl flex flex-col overflow-hidden border border-gray-100" style={{ height: '520px' }}>
            ...
            <div className="bg-gradient-to-r from-[#FFC107] to-[#FF8F00] px-4 py-3.5 flex items-center justify-between shrink-0">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="font-black text-dark text-sm leading-none">Aira</p>
                  <p className="text-dark/70 text-[11px] mt-0.5">Asisten Amoures Tour • Online</p>
                </div>
              </div>
            ...
            <div className="flex-1 overflow-y-auto px-4 py-4 bg-gray-50 space-y-0">
              {messages.map((msg) => (
                <div key={msg.id}>
                  <Message msg={msg} />
                  {/* Quick replies */}
                  {msg.quickReplies?.length > 0 && msg.id === messages[messages.length - 1]?.id && !typing && (
            ...
            {/* Input */}
            <form onSubmit={handleSend} className="px-3 py-3 bg-white border-t border-gray-100 flex items-center gap-2 shrink-0">
              <input
                ref={inputRef}
                type="text"
                value={input}
